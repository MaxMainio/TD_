...
{
std::string
supportedScaleFactorsText(NSArray<NSNumber*>* supportedScaleFactors)
{
	if (!supportedScaleFactors || supportedScaleFactors.count == 0)
	{
		return "none";
	}
	...
	for (NSNumber* n in supportedScaleFactors)
	{
		...
		result += std::to_string([n integerValue]);
		result += "x";
	}
...
bool
resolveSupportedScaleFactor(
	int requestedScaleFactor,
	NSInteger& resolvedScaleFactor)
{
	...
	NSInteger fallbackScaleFactor = 0;
	NSInteger largestScaleFactor =
	...
	for (NSNumber* n in supportedScaleFactors)
	{
		...
		if (value == requestedScaleFactor)
		{
			resolvedScaleFactor = value;
			return true;
		}

		if (value > largestScaleFactor)
		{
			largestScaleFactor = value;
		}

		if (value <= requestedScaleFactor &&
			(!hasFallback || value > fallbackScaleFactor))
		{
			fallbackScaleFactor = value;
			hasFallback = true;
		}
	}

	resolvedScaleFactor =
		hasFallback ? fallbackScaleFactor : largestScaleFactor;
...
int
AppleSuperResolution::scaleFactorFromMenuIndex(int scaleIndex) const
{
	switch (scaleIndex)
	{
		case 0:
...
const char*
AppleSuperResolution::inputTypeLabel(int inputTypeIndex) const
{
	return (inputTypeIndex == 1) ? "Video" : "Image";
}
	...
	int inputTypeIndex)
{
	@autoreleasepool
	{
		...
		NSInteger scaleFactor = 0;

		if (!resolveSupportedScaleFactor(requestedScaleFactor, scaleFactor))
		{
			std::cout
		...
		if (scaleFactor != requestedScaleFactor)
		{
			NSArray<NSNumber*>* supportedScaleFactors =
				[VTSuperResolutionScalerConfiguration supportedScaleFactors];
			...
			std::cout
				...
				<< "x is not supported. Supported scale factors: "
				<< supportedScaleFactorsText(supportedScaleFactors)
				<< ". Using "
				<< scaleFactor
				<< "x instead."
				<< std::endl;
	...
	void* reserved1)
{
	...
	if (strcmp(name, "Downloadmodel") == 0)
	{
		...
		{
			std::cout
				<< "Apple Super Resolution is not supported on this system."
				<< std::endl;
		...
		if (!resolveSupportedScaleFactor(lastRequestedScaleFactor, scaleFactor))
		{
			std::cout
				<< "No supported scale factors reported by VideoToolbox."
				<< std::endl;
		...
		if (scaleFactor != lastRequestedScaleFactor)
		{
			std::cout
				<< "Requested scale factor "
				<< lastRequestedScaleFactor
				<< "x is not supported. Supported scale factors: "
				<< supportedScaleFactorsText(supportedScaleFactors)
				<< ". Downloading "
				<< scaleFactor
		...
		{
			std::cout
				<< "Could not create SR config for model download. "
				<< "Input type: "
				<< inputTypeLabel(lastInputTypeIndex)
				<< ", scale factor: "
				<< scaleFactor
				<< "x."
				<< std::endl;
		...
		std::cout
			<< "Starting Apple Super Resolution model download. "
			<< "Input type: "
			<< inputTypeLabel(lastInputTypeIndex)
			<< ", scale factor: "
			<< scaleFactor
			<< "x. Supported scale factors: "
			<< supportedScaleFactorsText(supportedScaleFactors)
			<< "."
			<< std::endl;
	...
	void* reserved1)
{
	...
	int requestedScaleFactor =
		scaleFactorFromMenuIndex(scaleIndex);

	lastRequestedScaleFactor = requestedScaleFactor;
	lastInputTypeIndex = inputTypeIndex;
