...
Only network setup lives here. Segment table values come from the C++ Info DAT API.
"""
import td

# Private ownership/layout marker for Segment UV only.
_MARKER = "segmentUVInfoDATVersion"
_LAYOUT_VERSION = 1
...
def _matches(dat, owner):
    ...
    if dat.dock == owner and dat.fetch(_MARKER, None, search=False) in (0, _LAYOUT_VERSION):
        return True
    try:
...
def ensure_segment_dat(owner, reset_layout=False):
    """Reuse this owner's table, or let TouchDesigner choose a free new name."""
    expected_name = owner.name + "_segments"
    ...
    dat.store(_MARKER, layout_version if layout_version in (0, _LAYOUT_VERSION) else 0)
...
def dispatch(node_id, reset_layout=False):
    ...
    if owner is None or not owner.valid:
        return
    report = getattr(owner, "_segmentDockResult", None)
    if report is None:  # Python integration requires an installed Custom OP.
        return
    try:
        ensure_segment_dat(owner, reset_layout)
    except Exception as error:
        report("Could not create/repair Segment Info DAT: " + str(error))
    else:
        report("")
