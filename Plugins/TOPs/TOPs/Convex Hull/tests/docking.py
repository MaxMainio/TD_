...
class Node:
    ...
    def _hullDockResult(self, message):
...
class DockingTests(unittest.TestCase):
    def setUp(self):
        ...
        self.owner = self.network.create("convexhull", "convexhull1")
    ...
    def test_create_repeated_setup_and_repair(self):
        ...
        self.assertEqual(dat.name, "convexhull1_hulls")
    ...
    def test_old_layout_migrates_once_and_explicit_repair_resets_layout(self):
        dat = self.network.create("info", self.owner.name + "_hulls")
        dat.dock, dat.par.op.expr = self.owner, "me.dock"
        dat.store("convexHullDockedInfoDATVersion", 1)
    ...
    def test_duplicate_with_and_without_table(self):
        self.dispatch()
        duplicate = self.network.create("convexhull", "copy")
        copied = self.network.create("info", "copy_hulls")
        ...
        bare_copy = self.network.create("convexhull", "bare_copy")
    ...
    def test_name_collision_and_unrelated_docked_dat(self):
        collision = self.network.create("table", self.owner.name + "_hulls")
        other_owner = self.network.create("convexhull", "other")
