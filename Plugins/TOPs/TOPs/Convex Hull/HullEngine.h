...
{
    unsigned source = 0;
    int left = 0, bottom = 0, right = 0, top = 0; // Exclusive upper edges.
    double centroidX = 0, centroidY = 0, area = 0, perimeter = 0;
    uint64_t selectedPixels = 0, blobCount = 0, vertexCount = 0;
};

// Vertex coordinates are pixel indices; exposed positions use image-edge coordinates.
...
private:
    ...
    Image processImpl(Input input, Settings settings);
