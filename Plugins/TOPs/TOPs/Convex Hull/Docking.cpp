...
std::string pythonError()
{
    ...
    std::string error = "Could not schedule Hull Info DAT: ";
...
PyObject* dockResult(PyObject* self, PyObject* argument)
{
    try
    {
        ...
        TD::PY_GetInfo info;
        info.autoCook = false;
        auto* instance = static_cast<ConvexHullTOP*>(object->context->getNodeInstance(info));
    ...
    catch (...) { PyErr_SetString(PyExc_RuntimeError, "Hull docking status update failed"); }
    return nullptr;
}
PyMethodDef methods[] = {
    {"_hullDockResult", dockResult, METH_O, "Internal deferred docking status callback."},
...
{
    if (!Py_IsInitialized()) return "Hull Info DAT setup requires TouchDesigner's Python runtime";
    GIL gil;
    ...
    Ref arguments(Py_BuildValue("(sIO)", HullDockingScript, nodeId, resetLayout ? Py_True : Py_False));
