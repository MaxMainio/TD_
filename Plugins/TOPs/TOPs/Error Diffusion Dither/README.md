...
This folder includes a local copy of the maintained `TOPOutputHelper.h` dated
2026-09-07. The build does not depend on any parent or sibling source folders.
`OutputPacking.h` provides the local single-channel packing behavior.
...
non-finite input, buffer guards, and pixel packing. `DitherBenchmark [samples=9] [workers=3]` emits CSV for every
