...
void
SeamCarving::updateInternalEnergyBand(
	const cv::Mat& image,
...
) const
{
	...
	if (image.empty() || energy.empty() || width <= 0 || height <= 0)
		return;
	...
	for (int y = 0; y < height; ++y)
	{
		...
		seamX = std::clamp(seamX, 0, width - 1);

		int minX = std::max(0, seamX - radius);
		int maxX = std::min(width - 1, seamX + radius);
...
float
SeamCarving::computeCostAt(const cv::Mat& energy, const cv::Mat& cost, int x, int y) const
{
	const int width = energy.cols;
	const int height = energy.rows;

	const float e = energy.ptr<float>(y)[x];

	// Bottom row has no children below it.
	if (y == height - 1)
		return e;
...
void
SeamCarving::updateCumulativeCostDirty(
	const cv::Mat& energy,
	cv::Mat& cost,
	const std::vector<int>& seam
) const
{
	...
	if (energy.empty() || cost.empty() || width <= 0 || height <= 0)
		return;

	if (cost.cols != width || cost.rows != height)
	{
		computeCumulativeCost(energy, cost);
		return;
	}

	// Dirty masks: one row for current processing, one row for parents above.
	cv::Mat dirtyCurrent(height, width, CV_8UC1, cv::Scalar(0));

	const int radius = 3;

	// Initial dirty region: conservative band around the removed seam path.
	for (int y = 0; y < height; ++y)
	{
		int seamX = 0;
		...
		uint8_t* dirtyRow = dirtyCurrent.ptr<uint8_t>(y);
	...
	// Because cost flows bottom-to-top, process from bottom upward.
	for (int y = height - 1; y >= 0; --y)
	{
		...
		for (int x = 0; x < width; ++x)
		{
			...
			float oldCost = costRow[x];
			float newCost = computeCostAt(energy, cost, x, y);

			if (std::abs(newCost - oldCost) > epsilon)
			{
				costRow[x] = newCost;

				// Mark parents in the row above.
				if (y > 0)
				{
					uint8_t* parentRow = dirtyCurrent.ptr<uint8_t>(y - 1);
...
) const
{
	...
	if (
		costUpdate == CostUpdateMenuItems::Once ||
		costUpdate == CostUpdateMenuItems::Targeted
	)
	{
	...
	for (int i = 0; i < insertCount; ++i)
	{
		...
		if (costUpdate == CostUpdateMenuItems::Global)
		{
			if (!hasExternalEnergy)
				computeInternalEnergy(tempImage, tempEnergy);
		...
		if (costUpdate == CostUpdateMenuItems::Targeted)
		{
			if (!hasExternalEnergy)
				updateInternalEnergyBand(tempImage, tempSeam, tempEnergy);

			updateCumulativeCostDirty(tempEnergy, tempCost, tempSeam);
		}
...
SeamCarving::carveImage(const TD::OP_Inputs* inputs)
{
	...
	if (carveWidth > 0)
	{
		...
		for (int i = 0; i < removeCount; ++i)
		{
			...
			{
				if (!hasExternalEnergy)
					updateInternalEnergyBand(currentImage, seam, currentEnergy);

				updateCumulativeCostDirty(currentEnergy, currentCost, seam);
			}
		}
