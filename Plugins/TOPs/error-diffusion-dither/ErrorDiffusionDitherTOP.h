...
class ErrorDiffusionDitherTOP final : public TD::TOP_CPlusPlusBase
{
public:
    explicit ErrorDiffusionDitherTOP(TD::TOP_Context* context) : context_(context) {}
...
private:
    ...
    Dither::Engine engine_;
    std::string error_, warning_;
    std::array<float, 7> stats_{};
};
