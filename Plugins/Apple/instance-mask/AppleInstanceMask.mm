...
extern "C"
{
	...
	FillTOPPluginInfo(TD::TOP_PluginInfo* info)
	{
		...
		info->customOPInfo.opType->setString("Appleinstancemask");
		info->customOPInfo.opLabel->setString("Apple Instance Mask");
		info->customOPInfo.opIcon->setString("AIM");
	...
	{
		return new AppleInstanceMask(info, context);
	}
	...
	{
		delete (AppleInstanceMask*)instance;
	}
};

AppleInstanceMask::AppleInstanceMask(const TD::OP_NodeInfo* info, TD::TOP_Context* context)
	: myContext(context),
	  myExecuteCount(0)
{
}

AppleInstanceMask::~AppleInstanceMask()
{
	if (inputPool)
	{
...
void
AppleInstanceMask::getGeneralInfo(
...
void
AppleInstanceMask::ensurePool(size_t width, size_t height)
{
	if (inputPool &&
		inputPoolW == width &&
		inputPoolH == height)
...
void
AppleInstanceMask::runInstanceMask(
	...
	int instanceIndex)
{
	@autoreleasepool
	{
		...
		if (!error && request.results.count > 0)
		{
			...
			{
				if ([observation.allInstances containsIndex:instanceIndex])
				{
					instances =
						[NSIndexSet indexSetWithIndex:instanceIndex];
				}
				else
				{
					instances = observation.allInstances;
			...
			CVPixelBufferRef maskBuf =
				[observation generateMaskForInstances:instances
												error:&error];
...
void
AppleInstanceMask::setupParameters(
...
void
AppleInstanceMask::pulsePressed(
	const char* name,
	void* reserved1)
{
}
	...
	void* reserved1)
{
	...
	int instanceIndex = inputs->getParInt("Instance");
